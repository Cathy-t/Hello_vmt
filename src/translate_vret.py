...
def translate(video_name, src):
    ...
    parser.add_argument('--config', type=str, default='E:\graduation\code\demo\VMT\src\configs_vret.yaml')
...
def main(args, video_name, src):
    ...
    cp_file = os.path.join(checkpoint_path, 'best_model_vret.pth.tar')
    ...
    with torch.no_grad():
        ...
        video_feature = load_video_features(os.path.join(args.DATA_DIR, 'scene_node/' + video_name + '.npy'), args.MAX_VID_LENGTH)
        video_graph = load_video_features(os.path.join(args.DATA_DIR, 'scene_v_graph/' + video_name + '.npy'), args.MAX_VID_LENGTH)

        video = (video_graph.cuda(), video_feature.cuda())
        ...
        # en2zh
        pred_out, _ = beam_search_decode(model, encap, ensrccap_mask, video, None, args.maxlen,
                                         start_symbol=sos_idx,
                                         unk_symbol=unk_idx, end_symbol=eos_idx,
                                         pad_symbol=padding_idx, type='none')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--config', type=str, default='./configs_vret.yaml')
    parser.add_argument('--video_name', type=str, default='_UqnTFs1BLc_23_27')
    parser.add_argument('--src', type=str, default='a little girl is drinking water.')
